...
class SNRM(object):
    ...
    def __init__(self, dictionary, pre_trained_embedding_file_name, batch_size, max_q_len, max_doc_len, emb_dim,
        ...
        with self.graph.as_default():
            ...
            # logits_d1 = tf.multiply(logits_d1, 50)
            # logits_d1 = tf.reduce_mean(tf.multiply(self.q_repr, self.d1_repr), axis=1, keepdims=True)
            print(logits_d1)
            logits_d2 = tf.reduce_sum(tf.multiply(self.q_repr, self.d2_repr), axis=1, keepdims=True)
            # logits_d2 = tf.multiply(logits_d2, 50)
            # logits_d2 = tf.reduce_mean(tf.multiply(self.q_repr, self.d2_repr), axis=1, keepdims=True)
            print(logits_d2)
    ...
    def get_network_params(self, layer_sizes):
        ...
        weights_name = ['w' + str(i) for i in range(1, len(layer_sizes))] + ['w_out']
                ...
                weights[weights_name[i]] = \
                    tf.Variable(tf.truncated_normal(
                        [
                            filters[i],
                            1,
                            300 if i == 0 else layer_sizes[i-1],
                            layer_sizes[i]
                        ],
                        name=weights_name[i], stddev=0.1),
                    )
                biases[biases_name[i]] = tf.constant(0.1, shape=[layer_sizes[i]])
    ...
    def get_embedding_params(self, dictionary, dim, pre_trained_embedding_file_name=None):
        ...
        if pre_trained_embedding_file_name is None:
            ...
            print("initializing word embeddings with Glove..")
            ...
            init_matrix = np.zeros([dictionary.size(), dim])
        ...
        with tf.name_scope('embedding_layer'):
            with tf.name_scope(layer_name):
                with tf.variable_scope(layer_name, reuse=tf.AUTO_REUSE):
                    ...
                    emb = tf.reshape(emb, [-1, n_terms, 1, dim])
        return emb
