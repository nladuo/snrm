...
def get_class_num(_str):
    """ code for turn a specific string to a number """
...
for q in query_coll.find({"is_set": False}):
    # if get_class_num(str(q["_id"])) % 2 == 0:
        queries.append(q)
...
def insert_one_query(count, q):
    ...
    if count % 2 == 0:
        ...
        pair_wise_data_coll.insert(
            {"q": q["query"], "d1_id": negative_id1, "d2_id": positive_id1, "label": -1}
        )
        pair_wise_data_coll.insert(
            {"q": q["query"], "d1_id": negative_id2, "d2_id": positive_id2, "label": -1}
        )
